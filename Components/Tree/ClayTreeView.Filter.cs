...
public partial class ClayTreeView
{
    ...
    private async Task ApplyFilterAsync()
    {
        ...
        try
        {
            await RunBusyAsync("Поиск…", async () =>
            {
                ...
                if (_filterCapped)
                {
                    // SQL вернул max+1 совпадений для детекта переполнения — лишнее
                    // (последнее по порядку прихода) в дерево не пускаем, вместе с предками-сиротами.
    ...
    private static IReadOnlyList<ClayTreeNode> TrimOverflowMatch(IReadOnlyList<ClayTreeNode> flatNodes)
    {
        ...
        while (currentKey.Length > 0 && byId.TryGetValue(currentKey, out var ancestor))
        {
            ...
            currentKey = ClaySqlTreeDataSource.ToKey(ancestor.ParentId);
        }
