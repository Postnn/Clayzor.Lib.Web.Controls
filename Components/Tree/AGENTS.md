...
**Аудит TA:** [promts/_done/TA/TA0_README_audit_tree.md](promts/_done/TA/TA0_README_audit_tree.md).
...
- TA7 — `Dispose`: try/catch на JS-исключения; `BuildFilterColumns`: кэш через `??=`; `ResolveDb`: синхронизация `_resolvedCsName`.
...
**Сохранение раскрытости при reload (CTFR2–CTFR2.5):** `ReloadLevelAsync(null)` и `ReloadLevelAsync(parent)` симметричны: перед reload собираются childId→parentId раскрытых узлов (`CollectExpandedSnapshot`, parentId=null для корней) и paging-граница (`parent.Children.Count`), затем `RemoveFromIndex` очищает старый subtree из `_byId` и `_expanded`, после — восстановление сверху вниз через `RestoreExpandedAsync` с **bounded paging**: страницы догружаются до прежней границы. Moved/deleted child не вызывает полную загрузку и не оставляет stale Id в `_expanded`. Ветки вне reload scope не затрагиваются.
