...
**Сохранение раскрытости при root reload (CTFR2, CTFR2.1):** `ReloadLevelAsync(null)` симметричен non-root reload: перед `LoadRootsAsync()` собираются childId→parentId раскрытых узлов (`CollectExpandedSnapshot`), после — восстановление сверху вниз через `RestoreExpandedAsync` с **bounded paging**: если раскрытый потомок не попал в первую страницу, догружаются страницы пока он не будет найден (или `LoadedAllChildren`). Без дубликатов, cursor корректен для последующего пользовательского `LoadMore`.
