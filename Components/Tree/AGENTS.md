...
**Сохранение раскрытости при root reload (CTFR2, CTFR2.1, CTFR2.2):** `ReloadLevelAsync(null)` симметричен non-root reload: перед `LoadRootsAsync()` собираются childId→parentId раскрытых узлов (`CollectExpandedSnapshot`, parentId=null для корней) и paging-граница (`parent.Children.Count`), после — восстановление сверху вниз через `RestoreExpandedAsync` с **bounded paging**: если раскрытый потомок не попал в первую страницу, догружаются страницы до прежней границы. Moved/deleted child не вызывает полную загрузку уровня — restore останавливается на `maxChildren`. Root marker: `null` (структурно), без magic-string.
