...
### 1а. `ClayGroupingEngine`: потолок уровней — РАСПЛАНИРОВАН

Отдельный дефект, найденный при планировании GG. Касается ОБОИХ режимов. Разложен в план
...
### 2. Печать и выгрузка в Excel динамического грида — РАСПЛАНИРОВАНА

GF15 убирает нерабочий UI. Реализация разложена в отдельный план: **`GE0_README_dynamic_export.md`**
и промты `GE1`–`GE6`. GE6 откатывает GF15.
