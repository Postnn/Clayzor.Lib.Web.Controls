...
| **ClayButton** — обёртка `MudTooltip` + `MudIconButton` с авто-сбросом тултипа после клика. Заменяет пару `<MudTooltip><MudIconButton/></MudTooltip>`. Поддерживает опциональные `Href`/`Target` — при заданном `Href` рендерится как `<a>` (переход по ссылке) | — |
...
**Выполненные заплатки ClayTreeView (CTF1–CTF6):**
...
- SH9 — тесты и отчёт: `ClaySharedParamsDataTests` (4 теста валидации названия), `FullParamSet_RoundTrip_AllTypesPreserved` (1 round-trip), проверка SQL на 2008 R2. Отчёт `promts/_done/SH_share/SH9_report.md`. Всего 235 тестов (+32 в серии SH).
...
7. Настройки уровня приложения (из `appsettings`/`web.config`, через DI) — **другой** класс и другое имя (`Clay*DynamicSettings`, `ClayAppSettings`), в options экземпляра их не смешивать.
